 ...
 */
function getBackendGuidFromMaterial(
  material: THREE.Material,
): string | undefined {
  const entry = Array.from(SCENE_MATERIALS.values()).find(
...
export function removeMaterial(
  materialBackendGuidOrObject: string | THREE.Material,
): void {
  ...
  if (!entry) {
    console.warn(
      `Material with materialBackendGuid ${materialBackendGuid} not found`,
    );
    return;
  }
...
export function updateMaterial(
  ...
  GEOMETRY_MATERIALS.forEach((matGuid, geomGuid) => {
    if (matGuid === materialBackendGuid) {
      ...
      GEOMETRY_MATERIALS.set(geomGuid, materialBackendGuid);
    }
  });
}
 ...
 */
function applyMaterialToGeometry(
  geometryBackendGuid: string,
  material: THREE.Material,
): void {
 ...
 */
export function convertToLineMaterial(
  material: THREE.Material,
 ...
 */
export function getMaterialByGeometry(
  geometryBackendGuid: string,
): MaterialEntry | undefined {
